...
% engine parameters 
clear();
thetas(1,1)= -20; % Engine1 start of heat release (deg)
thetas(2,1)= 0;   % Engine2 start of heat release (deg)
thetas(3,1)= 10;  % Engine3 start of heat release (deg)
thetad(1,1) = 40; % Engine1 duration of heat release (deg)
thetad(2,1) = 40; % Engine2 duration of heat release (deg)
thetad(3,1)= 40;  % Engine3 start of heat release (deg)

r=10;       %compression ratio
gamma= 1.4; %gas const
q= 20.2;    % dimensionless total heat release Qin/P1V1 
...
[pmax3, id_max3] = max(save.press(:,3)); %Engine 2 max pressure

thmax1=save.theta(id_max1);%Engine 1 crank angle
thmax2=save.theta(id_max2);%Engine 2 crank angle
thmax3=save.theta(id_max3);%Engine 2 crank angle

m = 1.03*10^-3;
R = 83.14/29;

t1 = save.press(:,1).*save.vol(:,1)/(m*R);
t2 = save.press(:,2).*save.vol(:,2)/(m*R);
t3 = save.press(:,3).*save.vol(:,3)/(m*R);
...
eta_rat3 = eta3/(1-r^(1-gamma));

% output overall results
fprintf('                  Engine 1         Engine 2        Engine 3    \n');
fprintf(' Theta_start       %5.2f           %5.2f           %5.2f  \n', thetas(1,1), thetas(2,1), thetas(3,1));
fprintf(' Theta_dur         %5.2f           %5.2f           %5.2f  \n', thetad(1,1), thetad(2,1), thetad(3,1));
fprintf(' P_max/P_1         %5.2f           %5.2f           %5.2f  \n', pmax1, pmax2, pmax3);
fprintf(' Theta_max         %7.1f           %7.1f           %7.1f \n',thmax1,thmax2, thmax3);
fprintf(' Net Work/P1V1     %7.2f           %7.2f           %7.2f  \n', w1,w2, w3);
fprintf(' Efficiency        %5.3f           %5.3f           %5.3f  \n', eta1, eta2, eta3);
fprintf(' Eff. Ratio        %5.3f           %5.3f           %5.3f\n', eta_rat1, eta_rat2, eta_rat3);
fprintf(' Imep/P1           %5.2f           %5.2f           %5.2f \n', imep1, imep2, imep3);

%plot results

%set(gcf,'Units','pixels','Position', [50,50,1200,600]);
%subplot(1,2,1);
plot(save.theta,save.press(:,1),'-',save.theta,save.press(:,2),'--',save.theta,save.press(:,3),':','linewidth',2 )
...
figure();
%subplot(1,2,2);
plot(save.theta, t1, '-',save.theta,t2,'--', save.theta,t3,':','linewidth',2)
set(gca, 'fontsize', 18,'linewidth',2);
title('Temperature vs Crank Angle')
legend('Engine 1', 'Engine 2','Engine 3','Location','NorthWest')
xlabel('Theta (deg)','fontsize', 18)
ylabel('Temperature (K)','fontsize', 18)
%print -depsc heatreltemperature

% Plots Work vs Crank Angle
% figure();
% %subplot(1,2,3);
% plot(save.theta,save.work(:,1),'-',save.theta,save.work(:,2),'--', save.theta,save.work(:,3),':','linewidth',2)
% set(gca, 'fontsize', 18,'linewidth',2);
% %grid
% legend('Engine 1', 'Engine 2','Engine 3','Location','NorthWest')
% xlabel('Theta (deg)','fontsize', 18)
